...
import { 
  ...
  TrendingDown, BarChart3, Calendar, Wallet, Clock, Percent, HelpCircle
} from 'lucide-react';
...
const ResultsTab = ({
  ...
  const totalFee = results.results?.reduce((sum, tx) => sum + (tx.total_fee || 0), 0) || 0;
      ...
      <div className="card">
        ...
        <div className="results-summary">
          ...
          <div className="summary-card success">
            ...
            <div>
              ...
              <div className="summary-value">
                {results.summary?.transactions_processed || results.results?.length || 0}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fifo-explanation">
          ...
          <div className="tax-parameters">
            <div className="param">
              <span className="param-label">Annual Exclusion:</span>
              <span className="param-value">R{results.taxParameters?.annual_exclusion?.toLocaleString() || '40,000'}</span>
            </div>
            <div className="param">
              <span className="param-label">Short-term Rate:</span>
              <span className="param-value">{results.taxParameters?.short_term_rate || 18}%</span>
            </div>
            <div className="param">
              <span className="param-label">Long-term Rate:</span>
              <span className="param-value">{results.taxParameters?.long_term_rate || 10}%</span>
            </div>
          </div>
        </div>
                    ...
                    <div className="result-info">
                      ...
                      {result.total_fee > 0 && (
                        <div className="result-fee">
                          <Percent size={14} /> Fee: {formatCurrency(result.total_fee)} ({formatFeeRate(result.fee_rate || 0)})
                        </div>
                      )}
                    </div>
                  </div>
                        ...
                        <div className="tax-year">
                          ...
                          {result.total_tax > 0 && (
                            <span className="tax-amount">
                              • Tax: {formatCurrency(result.total_tax)}
                            </span>
                          )}
                        </div>
                      </>
                    ...
                    <table className="details-table">
                      ...
                      <tbody>
                        {result.matched_buys.map((buy, buyIndex) => (
                          <tr key={buyIndex}>
                            ...
                            <td>{formatCurrency(buy.net_proceeds)}</td>
                      ...
                      <tfoot>
                        <tr>
                          ...
                          <td><strong>{formatCurrency(result.matched_buys.reduce((sum, buy) => sum + (buy.fee || 0), 0))}</strong></td>
                          <td><strong>{formatCurrency(result.matched_buys.reduce((sum, buy) => sum + (buy.net_proceeds || 0), 0))}</strong></td>
